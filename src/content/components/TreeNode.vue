...
const isExpanded = computed(() => store.expandedIds.has(props.node.idx))
const rowPadding = computed(() => props.searchMode ? 8 : props.node.depth * INDENT_SIZE + 8)
const downloadLabel = computed(() =>
  props.node.isDir ? t.value('tree.downloadZip') : t.value('tree.downloadFile'),
)
const pathLabel = computed(() => {
  if (!props.searchMode) return ''
  const lastSlash = props.node.path.lastIndexOf('/')
  return lastSlash >= 0
    ? props.node.path.slice(0, lastSlash) || t.value('download.root')
    : t.value('download.root')
})
...
<template>
  ...
  >
    <template v-if="node.isDir && !searchMode">
    ...
    <span v-else class="tree-node-icon file">
      <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
        <path d="M2 1.75C2 .784 2.784 0 3.75 0h6.586c.464 0 .909.184 1.237.513l2.914 2.914c.329.328.513.773.513 1.237v9.586A1.75 1.75 0 0 1 13.25 16h-9.5A1.75 1.75 0 0 1 2 14.25Zm1.75-.25a.25.25 0 0 0-.25.25v12.5c0 .138.112.25.25.25h9.5a.25.25 0 0 0 .25-.25V6h-2.75A1.75 1.75 0 0 1 9 4.25V1.5Zm6.75.062V4.25c0 .138.112.25.25.25h2.688l-.011-.013-2.914-2.914-.013-.011z"/>
      </svg>
    </span>

    <span class="tree-node-meta" :title="node.path">
      <span class="tree-node-name">{{ node.name }}</span>
      <span v-if="searchMode" class="tree-node-path">{{ pathLabel }}</span>
    </span>
